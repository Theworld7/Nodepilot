...
async function handleProjectBind() {
  try {
    ...
    if (!selected) return
    const dirName = selected.split("/").pop() || selected.split("\\").pop() || "项目"
...
<template>
  ...
  <div class="version-row" :class="{ 'no-projects': projects.length === 0 }">
    <t-collapse v-model="activePanels">
      <t-collapse-panel :value="version.version">
...
.version-row.no-projects :deep(.t-collapse-panel__header-icon) {
  display: none;
}
